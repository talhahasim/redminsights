...
import { fetchMeta } from "@/lib/queries";
...
function AppInner() {
  ...
  const tab: Tab = pathname === "/resources" ? "resources" : "servers";
              ...
              <>
                ...
                {meta && (
                  <div className="mt-8 pt-4 border-t border-border text-xs text-muted text-center">
                    Updated {new Date(meta.cachedAt).toLocaleString()} — Refreshes
                    every hour — {meta.serversWithResources} servers tracked
                  </div>
                )}
              </>
            )}
